...
BATCH_COUNT=0
for root in /private/tmp/gc-formula-repair-e2e-claude.IFBdT6 /private/tmp/gc-formula-repair-e2e ~/gt/.beads/briefs; do
...
REPAIR_BEAD_COUNT=$(bd -C ~/gt/gascity-packs search "producer_contract: brief-producer-repair.v1" 2>/dev/null | grep -c '^gsp-' || true)
