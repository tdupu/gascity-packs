...
if grep -F '"kind":"upstream_repair_brief"' "$TMPDIR/upstream.jsonl" >/dev/null 2>&1 \
  && grep -F '"suspected_source":"math-city-work"' "$TMPDIR/upstream.jsonl" >/dev/null 2>&1 \
  && grep -F '"repair_target":"math-city-work verify-assignee gate"' "$TMPDIR/upstream.jsonl" >/dev/null 2>&1; then
...
if grep -F '"failure_class":"UNKNOWN_PROVENANCE"' "$TMPDIR/upstream.jsonl" | grep -F 'math-city-work verify-assignee gate' >/dev/null 2>&1; then
  record "unknown provenance does not blame math-city-work" fail "$(grep -F '"failure_class":"UNKNOWN_PROVENANCE"' "$TMPDIR/upstream.jsonl")"
else
  record "unknown provenance does not blame math-city-work" ok
fi
...
if grep -F "dispatch-provenance.v1" "$ROOT/skills/math-city-work/SKILL.md" >/dev/null 2>&1 \
  && grep -F "empty_assignee_after_verified_sling" "$ROOT/skills/math-city-work/SKILL.md" >/dev/null 2>&1 \
  && grep -F "bd create" "$ROOT/skills/math-city-work/SKILL.md" | grep -F -- "--type event" >/dev/null 2>&1; then
  record "math-city-work provenance and strand hooks documented" ok
else
  record "math-city-work provenance and strand hooks documented" fail "missing provenance hook"
fi
