...
they collide as **Open Questions for Taylor** (§10) rather than silently
picking a side:
...
27 `/Users/tdupuy` absolute paths (§7).** The `[steps.check]` conversion itself
is essentially done.
...
9. **De-hardcode absolute paths** (§7, its own bead) — sweep `/Users/tdupuy`.
