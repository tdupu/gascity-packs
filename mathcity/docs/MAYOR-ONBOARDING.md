...
- Flow: work → build-basic-briefed → brief → catch-no-brainer → `.pile` → brief-shuffle →
  `stack` → `/present-briefs` → Taylor adjudicates → verdict edge → `gc.publisher` → repo-side landing agent lands.
...
- The running city loads pack content from `~/repos/gascity-packs` (local-path import).
  ...
  changes go through repo-side landing agent (~/repos) + a rebuild/reload. Beads sync via the Dolt remotes.
- The repo-side landing agent does git landings (push/merge/branch-delete), each behind a fresh
  Taylor `authorize-git-operation`. Consensus with the repo-side landing agent before major work. Contact via
