...
govern what the *packs* may look like; these C-rules govern what the *city
...
| --- | --- | --- |
| Physics over politeness (GUPP) | **gap** | new C1.5 |
| Molecularize work in advance | **covered** | C3.1, C6.1 |
| One data plane (beads only) | **covered** | C10.3 (already PROPOSED) |
| Completion over uptime | **gap** | new C1.6 |
| Reusable workflow templates | **partial** | C4.1–C4.2 cover routing; new C4.4 covers the write-once/library discipline |
| Graceful degradation | **gap** | new C11.1–C11.2 |
| Patrols back off when idle | **gap** | new C1.7 |
| Never a single agent on a real process | **partial** | C9.1/C7.2 cover redundant *review*; new C9.3 covers redundant *execution* |
| Reliability is a dial | **covered** | C9.2 (already PROPOSED) |
| Formula library as durable/forkable inventory | **partial** | folded into new C4.4 |
...
| Fine-grained model tiering | **covered** | C8.1–C8.2 |
| Sandboxing (Taylor-flagged gap) | **confirmed zero coverage** | new Pillar C12 |

**Known caveat:** all new rule IDs below still use the `C` prefix, which
collides with the Computing domain's `C` prefix in
`mathcity/docs/rule-prefix-registry.md` (PP1.5 violation, previously
flagged, not yet resolved). Do not treat these IDs as final — they are
placeholders pending the prefix fix, tracked separately.

---

## Pillar C1 — Capacity & queueing

*Every rig has a user-set concurrency target; work waits, it is never lost.*

- **C1.1 Per-rig concurrency target N(R).** For each rig R there exists a
...
- **C1.2 Dispatched work is never lost — durable queue, immediate promotion.**
...
- **C1.3 (PROPOSED) Backpressure is surfaced, never silently accumulated.**
...
- **C1.4 (PROPOSED) No starvation; fairness across rigs.** No queued item
...
- **C1.5 (PROPOSED) Agents run their hook without waiting to be asked.** An
  ...
  Distinct from C1.2 (the scheduler's duty to promote queued work into a
  slot): C1.5 is the agent's own duty, once slotted, to act rather than idle
...
- **C1.6 (PROPOSED) The city optimizes for landing work, not for keeping
  processes alive.** N(R) (C1.1) describes desired concurrency, not a
  ...
  metric (C5.1); no molecule or session is kept alive past its work being
  done. Fail: sessions or molecules lingering with no eligible work, held
  open to preserve apparent throughput → **revise**.

- **C1.7 (PROPOSED) Patrols and orders back off when idle, and wake
...
- **C2.2 Near-duplicate instructions merge at intake.** If nearly identical
  ...
  into the brief (C7.1), and the merge is reported visibly ("MERGED with
...
- **C2.3 Supersession converges on in-flight work.** When a command is
  ...
  linked bead per P1.19), salvage completed steps that remain valid (C6.2),
...
## Pillar C3 — Planning structure

*Work arrives as prose; it runs as a dependency graph.*

- **C3.1 Work is automatically structured as PERT.** Accepted work is
  ...
  Parallelizable tasks are actually parallelized (subject to C1.1); serial
...
- **C3.2 Work is organized into epics.** Every task bead belongs to a parent
  epic (bd type `epic` — P5.3: real types only) grouping the user-level goal;
  epics carry the rollup view C5 reports on. Pass: no orphan task beads on
  multi-task work. Fail: related tasks with no common epic ancestor →
  **revise**.

- **C3.3 (PROPOSED) The critical path drives scheduling.** Among eligible
...
- **C3.4 (PROPOSED) Estimates are recorded and reconciled.** Each PERT task
...
## Pillar C4 — Routing & formula lifecycle

*Work goes to a formula that can do it; missing capability creates a
formula, through the front door.*

- **C4.1 Work routes to a capable formula.** When the user asks for work, the
...
- **C4.2 No capable formula ⇒ create one.** If no capable formula exists, the
  ...
  adversarial review of the design (C9.1), and the formula lands in the owned
...
- **C4.3 (PROPOSED) New formulas pass a dress rehearsal before joining
  ...
  brief (C7), before the router may select it for real work. (Origin: the
...
- **C4.4 (PROPOSED) Formulas are written once and reused as a library, not
  authored bespoke per dispatch.** A formula is composed once, versioned
  (commit-tracked, per C7.3 provenance), and instantiated repeatedly; the
  formula set is a durable, browsable inventory (`README-formulas.md` is the
  existing index) that a router (C4.1) and a human can search before writing
  a new one-off workflow. Hand-assembling a recurring multi-step job from
  scratch a second time, instead of formularizing it, is a C4.2 violation in
...
## Pillar C5 — Observability

*The user can always see what is and is not being worked on.*

- **C5.1 Total visibility of work state.** At all times the user can
  ...
  **in progress** (and on which rig/molecule), what is **blocked** (and on
  ...
  view → **fail** (invisible work is lost work waiting to happen, C1.2).

- **C5.2 Molecules report steps and percentage.** Every molecule declares its
  ...
  denominator is not a status. Pass: the C5.1 view shows x/y (z%) for every
  in-progress molecule. Fail: a molecule in progress with unknown total or
  unreported position → **revise**.

- **C5.3 (PROPOSED) The waiting-on-you list is explicit.** Everything blocked
...
- **C5.4 (PROPOSED) ETAs derive from the PERT chart.** The C5.1 view shows,
  per epic, a projected completion derived from remaining critical-path
  estimates (C3.4) and current N(R) capacity — clearly labeled as an
  estimate. Pass: epics display a derived ETA. Fail: none (advisory rule;
  no fail state until adopted with teeth).

## Pillar C6 — Interruptibility & salvage

*Stopping work must be cheap; nothing done is thrown away.*

- **C6.1 Work is interruptible at step boundaries.** Molecules checkpoint at
  ...
  stop, supersession (C2.3), crash, or budget halt (C8.3) — loses at most the
...
- **C6.2 Interrupted work is salvaged, not abandoned.** An interrupted
  ...
  **resumption brief** (what's done, what's next, what changed), a released
  ...
  (C10.1). A later molecule — or the same one resumed — picks up from the
...
## Pillar C7 — Artifacts & briefs (definition of done)

*Every dispatch returns a thing and the standard for judging the thing.*

- **C7.1 Every dispatch returns an artifact and a judging brief.** All work
...
- **C7.2 (PROPOSED) The producer is not the judge.** The artifact-vs-brief
...
- **C7.3 (PROPOSED) Artifacts carry provenance.** Every artifact records:
  originating bead, formula + version (commit), model(s) used per tier
  (C8.2), and input references — enough to re-derive or audit it (the
...
## Pillar C8 — Resource economy

*Tokens are the city's fuel; spend them where judgment lives.*

- **C8.1 Work is optimized for token usage.** Formulas and plans are shaped
  to minimize token spend: no recomputation of work already done (C2), no
  re-reading of unchanged context a checkpoint already summarized, no
  high-tier model invoked where a low tier suffices (C8.2), and PERT
  decomposition (C3.1) sized so each molecule carries only the context its
...
- **C8.2 Model tiering: high plans, low executes.** High-capability models
  are used for planning, design, decomposition, adversarial review, and
  judgment (C7.2, C9.1); low-cost models execute well-specified mechanical
  steps. The tier used is recorded per step (feeds C7.3 provenance and C3.4
...
- **C8.3 (PROPOSED) Budgets and runaway detection.** Every molecule carries a
  token budget (from its C3.4 estimate); actuals are metered against it. A
  molecule exceeding budget halts **loudly** at the next step boundary
  (C6.1) with an escalation naming the overrun and the options (raise
...
- **C9.1 All plans are adversarially reviewed.** Every plan is checked by an
  adversarial review agent **before execution**: the reviewer's stance is to
  refute — hunt for root-cause misses (P1.17), capacity violations (C1),
  idempotency holes (C2), boundary breaks (P2/P3), silent-failure paths
  (P6.1), and brief-lessness (C7.1). Verdicts use the standard vocabulary
...
- **C9.2 (PROPOSED) Review depth scales with blast radius.** Trivial,
...
- **C9.3 (PROPOSED) Non-trivial live-system work is never dispatched to a
  ...
  steps carved out by C9.2), the city runs at least two agents with
  overlapping responsibility — a producer and an independent checker (C7.2),
  ...
  C7.2/C9.1's redundant **review**: a lone reviewer catching a lone
  producer's mistake after the fact is C7.2; this rule is about not staking
...
## Pillar C10 — Repository hygiene

*Every rig is maintained to research-mathematics standards. No slop.*

- **C10.1 Clean worktrees and branches, always.** Every repository the city
...
- **C10.2 (PROPOSED) Molecule garbage collection.** Every molecule cleans up
  its working directory/worktree on exit — success, failure, or interrupt
  (C6.2 salvage commits first, then cleans) — and a periodic sweep order
...
- **C10.3 (PROPOSED) State lives in beads, not scratch files.** Durable
  ...
  scope; C1.2 durability depends on it). Pass: no work state exists only in
...
## Pillar C11 — Resilience & graceful degradation
...
- **C11.1 (PROPOSED) Every worker role functions degraded, not dead, when a
  ...
  blocked, and it is blocked *loudly* (C5.3-style visible waiting-state),
...
- **C11.2 (PROPOSED) No role is a single point of failure for the whole
...
- **C12.1 (PROPOSED) An agent states its execution boundary before acting,
...
- **C12.2 (PROPOSED) No mutating action outside the declared boundary
  without explicit escalation.** An action that would touch a path, host,
  credential, or service outside the declared sandbox (C12.1) is not
...
- **C12.3 (PROPOSED) Sandbox violations fail loud, never silent-succeed
...
- **C12.4 (PROPOSED) Sandbox boundary is verified, not assumed, before
  ...
  (C12.1) immediately before acting — not trust that a path "looks right."
...
- No duplicate compute from duplicate dispatch — pre-sling check, merge at
  intake, converge on supersession (C2.1–C2.3 / P1.21).
- No multi-step work without a PERT decomposition under an epic (C3.1–C3.2).
...
- No invisible work: full queue/in-progress/blocked view, steps x/y (z%)
  per molecule (C5.1–C5.2).
- No interruption that loses completed steps; no abandonment without a
  resumption brief (C6.1–C6.2).
- No work closed without artifact + brief + verdict (C7.1).
- No high-tier tokens on mechanical steps; no low-tier planning; tier
  recorded (C8.1–C8.2).
- No plan executes without an adversarial, non-author review (C9.1); no
  real-system mutation on one agent's uncorroborated pass alone (C9.3).
- No slop: clean worktrees, purposeful branches, on every rig, at every
  molecule exit (C10.1).
- No agent sits idle on hooked work waiting for a nudge (C1.5); no patrol
  polls forever at a fixed rate with nothing to do (C1.7).
- No dependency outage silently halts unrelated work (C11.1); no single
  role's absence freezes the whole city (C11.2).
- No mutating action outside a stated sandbox boundary without stop-and-ask;
  no destructive op against an unconfirmed target (C12.1–C12.4).
...
2. **What counts as "nearly identical" for C2.2 intake merging?** Candidate:
...
3. **Step-count granularity for C5.2.** PERT tasks (C3.1) are the natural
   denominator; needs a convention for molecules whose formula steps and
   PERT tasks differ.
4. **Who staffs the judge (C7.2) and the adversarial reviewer (C9.1)?**
   Candidate: review-synthesizer-shaped roles at high tier (C8.2), distinct
   from the operator pool so capacity pressure can't squeeze out review.
5. **Budget numbers for C8.3.** Initial per-molecule defaults, and whether
   budgets derive from C3.4 estimates automatically or are set per epic.
6. **Which PROPOSED rules to adopt, adapt, or reject** — each was written
   to be individually strikeable.
7. **C-prefix collision (carried forward, unresolved).** Every rule ID in
   this document, including all rules added in the 2026-07-23 principles-
   adaptation pass, uses `C` — which the prefix registry already assigns to
   the Computing domain (`subdomains/computing/POLICY.md`). This document
   cannot be adopted, and no ID here is safe to cite elsewhere, until the
   collision is resolved (candidate: rename this domain's prefix, e.g. `CO`
   or `CT`, and renumber).
8. **Minimum crew size for C9.3.** "At least two agents" is stated but not
...
9. **Does sandboxing (Pillar C12) need its own trinity?** Given the severity
...
10. **Evidence for C12.** Drafted from Taylor's stated observation ("many
    ...
    sandbox incidents, citing them here would strengthen C12's origin notes
    the way C1.3/C10.2 cite concrete precedents.
...
- Brief-system POLICY.md (B/G-rules) — the human-adjudication pipeline C5.3
  surfaces
...
- Steve Yegge, ["Welcome to Gas Town"](https://steve-yegge.medium.com/welcome-to-gas-town-4f25ee16dd04)
  (Jan 2026) — GUPP/physics-over-politeness (C1.5), completion-over-uptime
  (C1.6), patrol backoff (C1.7), graceful degradation (C11)
- Steve Yegge, ["Welcome to Gas City"](https://steve-yegge.medium.com/welcome-to-gas-city-57f564bb3607)
  (Apr 2026) — formula library discipline (C4.4), redundant execution (C9.3),
  reliability-as-a-dial (already reflected in C9.2)

## Change log

### 2026-07-23 — Principles adaptation pass (Gas Town / Gas City)
Added Pillars C11 (Resilience & graceful degradation) and C12 (Sandboxing &
environment discipline, Taylor-directed — a confirmed zero-coverage gap);
added C1.5 (physics over politeness), C1.6 (completion over uptime), C1.7
(patrol backoff), C4.4 (formula library discipline), C9.3 (redundant
...
Carries forward the still-unresolved `C`-prefix collision with the
Computing domain (Open question 7) — none of today's new IDs are exempt
from that fix.
...
added by Claude for grilling: backpressure (C1.3), anti-starvation (C1.4),
critical-path scheduling (C3.3), estimate reconciliation (C3.4), formula
dress rehearsal (C4.3), waiting-on-human list (C5.3), PERT ETAs (C5.4),
independent judging (C7.2), provenance (C7.3), budgets/runaway detection
(C8.3), tiered review depth (C9.2), molecule GC (C10.2), state-in-beads
(C10.3). Not yet grilled; no rule here is standing policy until adopted.
