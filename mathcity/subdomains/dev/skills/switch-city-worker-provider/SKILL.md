...
- Run from the city root as an outside agent helping Taylor.
...
- Do not hand-edit city config as a pack fix. If Taylor explicitly authorizes
...
is `gascity-packs/mathcity.codex-worker`.
...
by a provider token prompt and Taylor wants one deterministic machine worker
to run under Codex.
...
Use this step only with explicit current-session authorization from Taylor.
Set:
...
Rollback is mandatory unless Taylor explicitly extends the test window.
...
If rollback verification fails, stop and escalate to Taylor before running more
city work.
