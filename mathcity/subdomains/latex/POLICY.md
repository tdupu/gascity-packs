...
Every BibTeX key has the form `AuthorYEARfirstword` — capitalized author surname, 4-digit year, then the first significant word of the title, lowercase (e.g. `Smith2010modular`, `Serre1973course`).
...
(c) a "general knowledge" note — the result is standard enough to appear in at least one textbook, and that textbook is cited (pinpoint, per LX2).
...
The root `.tex` file compiles to PDF with zero errors on a standard engine (pdflatex/xelatex/lualatex), including the BibTeX/biber pass.
