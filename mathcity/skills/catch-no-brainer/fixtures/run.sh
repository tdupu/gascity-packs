...
classify_brief() {
  ...
  if echo "$files" | grep -E "$server_re" >/dev/null 2>&1; then
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":false,"category":null,"reason":"cat-E-server-touching","compact_eligible":false,"confidence":1.0,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  if echo "$files" | grep -E "$userskill_re" >/dev/null 2>&1; then
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":false,"category":null,"reason":"user-skill-touching-override","compact_eligible":false,"confidence":1.0,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  if [[ -n "$cap_block" ]]; then
    ...
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":false,"category":"capability-blocker","reason":"%s","compact_eligible":false,"confidence":1.0,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f" "$reason"
    return 0
  fi
  ...
  if [[ "$branch_ok" == "1" ]] \
    ...
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":true,"category":"stale-branch","reason":null,"compact_eligible":true,"confidence":0.9,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  if [[ "$verdict" == "DEFER" ]] && ([[ "$status" == "HELD" ]] || [[ "$existing_state" == "HELD" ]]); then
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":true,"category":"defer-ratify-held","reason":null,"compact_eligible":true,"confidence":0.9,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  if [[ "$verdict" == "CLOSE" ]] && [[ -n "$merged_commit" ]]; then
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":true,"category":"close-done-cited-commit","reason":null,"compact_eligible":true,"confidence":0.9,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  if [[ "$verdict" == "CONFIRM" ]] && [[ -n "$exec_proof" ]]; then
    printf '{"brief_path":"%s","bead_id":null,"no_brainer":true,"category":"execution-confirmation-proof","reason":null,"compact_eligible":true,"confidence":0.9,"proposed_registry_extension":null,"requires_taylor_adjudication":false,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f"
    return 0
  fi
  ...
  printf '{"brief_path":"%s","bead_id":null,"no_brainer":"candidate","category":null,"reason":null,"compact_eligible":false,"confidence":0.5,"proposed_registry_extension":"%s","requires_taylor_adjudication":true,"classified_at":"2026-07-24T00:00:00Z"}\n' "$f" "$proposed"
}
...
for f in stale-branch-A.md stale-branch-B.md stale-branch-C.md server-touching.md novel-shape.md capability-blocker.md defer-ratify-held.md close-done-cited-commit.md execution-confirmation-proof.md; do
  ...
  if command -v jq >/dev/null 2>&1; then
    if ! echo "$out" | jq -e '
      ...
      and (.requires_taylor_adjudication | type == "boolean")
