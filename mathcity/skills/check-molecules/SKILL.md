...
description: Enumerate EVERY molecule/bead that push-the-fleet could dispatch right now — the full ready, unblocked, top-level candidate set across all rigs, ranked by priority. Writes the complete ranked list to a file at ~/gt/molecules and prints ONLY the top 20 to the terminal, then asks whether there's anything in particular you'd like to see. Use when the user says "check molecules", "check-molecules", "what could push-the-fleet dispatch", "show me the dispatch candidates", "list the ready molecules", "what's dispatchable", "molecule queue", or wants a full inventory of feedable work before running push-the-fleet. Read-only — enumerates and reports; never dispatches (that's push-the-fleet) and never adjudicates.
---
...
- **Full ranked list → `~/gt/molecules`** (a file, overwritten each run).
...
```bash
bash ~/gt/.claude/skills/check-molecules/scripts/enumerate-molecules.sh
```

(or the pack path
`~/repos/gascity-packs/mathcity/skills/check-molecules/scripts/enumerate-molecules.sh`).

Env overrides: `MOLECULES_FILE` (default `~/gt/molecules`), `MOLECULES_TOP`
(default `20`), `GC_CITY` (default `~/gt`).

## What counts as a candidate (matches push-the-fleet's filter)

The script sweeps `bd ready` in **every rig** (each dir under `~/gt` with a
...
`~/gt/PRIORITIES.md` exists, beads whose id is listed under its **P0** section
...
> Full list written to `~/gt/molecules` (N candidates). Top 20 shown above.
