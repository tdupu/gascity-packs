Translate `{{plan_path}}` and optional context `{{context_path}}` into a
Superpowers implementation convoy. Record `{{decomposition_path}}` if the
caller provided a decomposition artifact path.
