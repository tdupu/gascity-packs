...
| Agent skill / slash-command | `skill-creator-math` | `~/repos/gascity-packs/mathematics/skills/<name>/` |
...
- **Pack**: one of `mathematics`, `gascity`, `superpowers`, `pr-pipeline`,
...
- **File location**: `~/repos/gascity-packs/<pack>/formulas/<name>.toml`
  — note mathematics uses the bare `.toml` extension; other packs often
...
### 5. Run the surface-test quality gate (mathematics formulas only)
...
**Scope note**: this gate currently covers only `mathematics/formulas/`.
...
- Does NOT run the surface test for non-mathematics packs (test is not yet
