...
if [ -n "$EXPECTED_VERDICT" ] && [ "$recorded_verdict" != "$EXPECTED_VERDICT" ]; then
  fail_misaligned "recorded verdict '$recorded_verdict' does not match expected '$EXPECTED_VERDICT' for bead $SOURCE_BEAD — supersede the decision record via mathematics.record-decision if the verdict changed"
fi
